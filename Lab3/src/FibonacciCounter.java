...
public class FibonacciCounter {
   ...
   * the current.
   * <p>
   * An IllegalArgumentException will be thrown if the countvalue is equal to zero because if it is
   * equal to zero, a decrement would make it negative. Note that an exception is not thrown for a
   * countvalue below zero because an object cannot created with a counvalue below 0 in the first
   * place due to an IllegalARgumentException that would be thrown by the constructor.
   *
   ...
   * <p>
   * An IllegalArgumentException is thrown if the number to calculate is bigger than
   * Integer.MAX_VALUE.
   *
   * @return the value at the current countValue. Math.round is used to ensure the result is an
   * integer, since using a double for instance would not result in enough decimal places for the
   * program to realize Binet's formula guarantees an integer.
   */
  public long fib() {
    long big = Math.round(((1 / Math.sqrt(5)) * (Math.pow((1 + Math.sqrt(5)) / 2, this.countValue))
    ...
    if (big < Integer.MAX_VALUE) {
      ...
      throw new IllegalArgumentException("Sequence number is too large.");
    }
  }
}
